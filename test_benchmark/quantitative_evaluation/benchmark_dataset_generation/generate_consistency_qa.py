...
def parse_args():
    ...
    python3 test_benchmark/quantitative_evaluation/benchmark_dataset_generation/generate_consistency_qa.py\
        --gt_caption_folder /home/tony/MiniGPT-4/daisee_captions/test_filter_cap.json\
...
def escape_inner_quotes(text):
    """
    Escapes single and double quotes that are directly between alphanumeric characters.

    Parameters:
        text (str): The input string to process.

    Returns:
        str: The processed string with inner quotes escaped.
    """
    # Pattern to find single or double quotes between word characters
    pattern = r'(?<=\w)(["\'])(?=\w)'

    # Replacement pattern: prefix the quote with a backslash
    replacement = r'\\\1'

    # Use re.sub to replace the pattern with the escaped quote
    escaped_text = re.sub(pattern, replacement, text)
...
def annotate(gt_file, caption_files, output_dir):
    ...
    for file in caption_files:
        ...
        try:
            ...
            response_message = escape_inner_quotes(completion["choices"][0]["message"]["content"].replace('\'','\"'))
    ...
    if ann.endswith('.json') and 'engagement' in ann:
        ...
        with open(ann, mode='r', encoding='utf-8-sig') as f:
            ...
            for pair in labels['annotations']:
                gt_captions[pair['video_id']] = pair['caption']
    else:           
